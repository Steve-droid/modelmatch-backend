...
def register(payload: UserCreate, db: Session = Depends(get_db)) -> UserOut:
    ...
    return UserOut.from_user(service.register_user(db, payload.email, payload.password))


@router.post("/login", response_model=TokenOut, dependencies=[Depends(limit_auth_requests)])
...
def me(response: Response, current_user: User = Depends(get_current_user)) -> UserOut:
...
@router.post("/google/challenge", response_model=GoogleChallengeOut,
             dependencies=[Depends(google.require_google_origin), Depends(limit_auth_requests)])
